...
class NxtTrendzStore {
   ...
   getPrimeDeals = async (): Promise<unknown> => {
      ...
      const response = this.nxtTrendzServices.getPrimeDeals(apiUrl)
   ...
   @action.bound fetchProductDetails = async (id: string): Promise<unknown> => {
      const url = `https://apis.ccbp.in/products/${parseInt(id)}`
      const response = this.nxtTrendzServices.getProductItemDetails(url)
      return bindPromiseWithOnSuccess(response)
         .to(
            status => {
   ...
   > => {
      ...
      const activeRatingId = this.activeRatingId
      const activeCategory = this.activeCategory

      // this.apiStatus = apiStatusConstants.inProgress
      ...
      const response = this.nxtTrendzServices.getAllProducts(apiUrl)
      return bindPromiseWithOnSuccess(response)
         .to(
            status => {
